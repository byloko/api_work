...
//dd($update_rec->address);
 ...
 //dd($record_insert->order_firebase_chat_id); 
    ...
    if($request->user_id){
		...
		$data['account_number'] 	    = !empty($value->account_number) ? $value->account_number : '';
		$data['bank_holder_name'] 	    = !empty($value->bank_holder_name) ? $value->bank_holder_name : '';
		$data['ifsc_code'] 	        = !empty($value->ifsc_code) ? $value->ifsc_code : '';
		$data['branch_code'] 	    = !empty($value->address) ? $value->branch_code : '';
		$data['bank_name'] 	        = !empty($value->address) ? $value->bank_name : '';
		$data['online_offline_status'] 	    = $value->online_offline_status;
	...
	public function app_payment_orders_update(Request $request){
		...
		if(!empty($update_record)){
		    ...
		    $update_record->is_flag               = "2";
		  	...
		  	if($update_record->payment_status == 1){
		  		$update_record_uers = User::find($update_record->user_mearchant_id);
		  		//$AddAmount    = $update_record_uers->amount + $update_record->orders_total_price;
		  		$Gunkar = $update_record->orders_total_price * 10;
		  		// dd($Gunkar);
		  		$Getpercentage = $Gunkar / 100;
		  		//dd($Getpercentage);
		  		$AddAmount    = $update_record_uers->amount + $Getpercentage;
		  		//$AddAmount    = $update_record_uers->amount + 100 / $update_record->orders_total_price * 10;
             	$update_record_uers->amount = $AddAmount;
                $update_record_uers->save();
		  	}
...
// Resend OTP End
	...
	public function app_add_money_wallet(Request $request){
	  ...
	  if (!empty($request->amount) && !empty($request->user_id)) {
		$update_record = User::find($request->user_id);
		if(!empty($update_record)){
	       
	        if($request->money_type == 0){	
    	    	$AddAmount = $update_record->amount + $request->amount;
    			$update_record->amount = $AddAmount;
    	        $update_record->save();   
    	    $json['status'] = 1;
    		...
    		if($update_record->amount >= $request->amount){
        		if($request->money_type == 1){
        		    // $LessAmount = $update_record->amount - $request->amount;
        			// $update_record->amount = $LessAmount;
        	        // $update_record->amount = $request->amount;
        	        $update_record->save();
        	        $json['status']  = 1;
        	    	$json['message'] = 'Withdraw Money wallet successfully.';
        		}
    		
    	    $json['user_data'] = $this->getProfileUserWallet($update_record->id);

    	    $user_insert                   = new UsersWalletDetailsModel;
            $user_insert->user_id          = $request->user_id;
            $user_insert->amount_transfer  = $request->amount;
	...
	public function getProfileUserWallet($id){
	    $user 				    = User::find($id);
		$json['user_id']		= $user->id;
		$json['name'] 			= !empty($user->name) ? $user->name : '';
		$json['lastname'] 		= !empty($user->lastname) ? $user->lastname : '';
...
// 		$option_main = array();
// 		foreach($user->get_users_wallet_details as $option)
// 		{
// 			$data = array();
// 			$data['id'] 	         = $option->id;
// 			$data['user_id'] 	     = $option->user_id;
// 			$data['amount_transfer'] = $option->amount_transfer;
// 			$data['money_type']      = $option->money_type;
// 			$data['money_date']      = $option->money_date;
// 			$data['money_status']    = $option->money_status;
// 		 // $data['money_status'] = !empty($option->money_status) ? $option->money_status : '0';
// 			$option_main[] = $data;
// 		}

// 		$json['users_wallet_details'] 		= $option_main;
	...
	public function app_wallet_details_list(Request $request){
	  $getresultCount = User::where('id', '=', $request->user_id)->count();
	    ...
	    $getresult = UsersWalletDetailsModel::where('user_id', '=', $request->user_id)->orderBy('id', 'DESC')->get();
