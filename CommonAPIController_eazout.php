...
class CommonAPIController extends Controller {
    ...
    public function app_checkout_cart_empty_opp(Request $request){
...
// Add To Cart End
	...
	public function app_checkout_cart_list(Request $request){
		...
		$getresult = Cart::where('user_id', '=', $request->user_id)->get();
		foreach ($getresult as $value) {
			$data['id']				   = $value->id;
...
// Add Address End
 	...
 	public function app_rating_add(Request $request){
 		...
 		if(!empty($getUserCount) && !empty($getDriverCount)){
	 		...
	 		$record_insert->rating_title       = trim($request->rating_title);
	 		$record_insert->rating_description = trim($request->rating_description);
...
// if(!empty($getServiceCount)){
	...
	public function app_place_order_add_old_w(Request $request){
		// user_id
		// checkout_total
		$getUserCount = User::where('id', '=', $request->user_id)->count();
...
// order add
	...
	public function app_order_accept_reject(Request $request){
		$update_accept_reject = User::find($request->user_id);
		...
		//	$update_accept_reject->trip_id             = trim($request->trip_id);
			...
			$json['status']  = 1;
			$json['message'] = 'Your order successfully change.';
		}else{
			$json['status']  = 0;
			$json['message'] = 'Your order has change.';
		}
		echo json_encode($json);
	}

	public function app_place_order_add_old_working(Request $request){

		// user_id
		// checkout_total   => checkout_total_all
		// orders_name
		// order_date_time 2021-05-28 06:20:22  2021-05-28 06:20:22 YYYY-MM-DD HH:MM:SS
		// order_status  0:Pending,1:Processing, 2:Complete
		$getUserCount = User::where('id', '=', $request->user_id)->count();
		if(!empty($getUserCount)){
			$record_insert = new OrderModel;
			if(!empty($record_insert)){
				$record_insert->user_id            = trim($request->user_id);
				$record_insert->checkout_total_all = trim($request->checkout_total);
				$record_insert->orders_name 	   = trim($request->orders_name);
				$record_insert->order_date_time    = trim($request->order_date_time );
				$record_insert->order_status	   = trim($request->order_status);
				$record_insert->save();
				...
				$json['status']  = 1;
				$json['message'] = 'Carts Successfully.';
				$json['result'] = $this->getOrderList($record_insert->id); 
			}else{
				$json['status'] = 0;
				$json['message'] = 'Id Incorrect.';
			}
		...
		//dd($update_record->users_address_id);
			if(!empty($update_record)  && !empty($UsersAddressCount)){
				...
				//dd($getUsersAddress);
					...
					->where(DB::raw("(111.111 * DEGREES(ACOS(LEAST(1.0, COS(RADIANS(users.lat)) * COS(RADIANS(".$getUsersAddress->latitude.")) * COS(RADIANS(users.lang - ".$getUsersAddress->longitude.")) + SIN(RADIANS(users.lat)) * SIN(RADIANS(".$getUsersAddress->latitude."))))) * 0.621371)"), "<=", 5)
